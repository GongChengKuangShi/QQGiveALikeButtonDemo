                    ...
                    maxFingers:(NSInteger)maxFingers
{
    ...
    CGSize size = [num sizeWithFont:kNumFont maxSize:CGSizeMake(100.f, 20.f)];//计算文字的宽度
...
- (void)thumbButtonClick
{
    if (_count >= self.maxFingers && _maxFingers != 0) {
        // 点赞失败
        if (self.delegate && [self.delegate respondsToSelector:@selector(zf_thumbFail)]) {
            [self.delegate zf_thumbFail];
        }
        else if (self.zf_thumbFailBlock) {
            self.zf_thumbFailBlock(YES);
        }
        else {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(zf_thumbButtonClick:)]) {
            [self.delegate zf_thumbButtonClick:zans];
        }
        else if (self.zf_thumbClickBlock) {
            self.zf_thumbClickBlock(zans);
        }
        else {
            NSLog(@"未设置代理或者回调，如果要获取点赞结果请设置");
        }
